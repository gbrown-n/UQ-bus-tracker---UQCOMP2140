    ...
    */
    ...
    async function loadData(routeInput, dateInput, timeInput) {
        ...
        async function merge_details(selected_trip, uq_trips, uq_routes, uq_stops) {
            ...
            const out =  await Promise.all(selected_trip.map(async trip => {
                ...
                if (!liveTime || liveTime[0] === 'No Live Data') {
                    liveTime = 'No Live Data';
                } else {
                    let liveTimeStr = liveTime[0].concat("000");
                    print(liveTimeStr);
                    liveTime = new Date(parseInt(liveTimeStr));
                    liveTime = liveTime.toTimeString().slice(0,8);
                }
    ...
    async function search() {
        ...
        let date = () => {
            ...
            /* Error Checking*/
            if (enteredDate[4] !== '-' || enteredDate[7] !== '-' ||enteredDate.length > 10 || isNaN(enteredDate.slice(0,4) || isNaN(enteredDate.slice(4,6) || isNaN(enteredDate.slice(6,7))))){
                print(ERR_DATE);
                return date()
            } else {
                return enteredDate;
